...
CVP_ENV_MANAGED_VARS="$CVP_PROFILES/.env-managed"
...
# Validate an environment variable name before passing it to export or unset.
_cvp_valid_env_key() {
...
# Persist every valid key seen in any profile. The ledger is append-only so a
# variable remains cleanable after the profile that introduced it is removed.
_cvp_record_managed_vars() {
  _cvp_setup
  local tmp sorted key pair file
  tmp=$(mktemp)
  sorted="${tmp}.sorted"

  if [[ -f "$CVP_ENV_MANAGED_VARS" ]]; then
    while IFS= read -r key || [[ -n "$key" ]]; do
      _cvp_valid_env_key "$key" && printf '%s\n' "$key" >> "$tmp"
    done < "$CVP_ENV_MANAGED_VARS"
  fi

  for file in "$CVP_PROFILES"/*.env; do
    [[ -f "$file" ]] || continue
  ...
  LC_ALL=C sort -u "$tmp" > "$sorted"
  mv "$sorted" "$CVP_ENV_MANAGED_VARS"
  rm -f "$tmp"
}
...
_cvp_name=$(_cvp_resolve_profile) || { unset -f _cvp_resolve_profile _cvp_unset_key 2>/dev/null; unset _cvp_dir _cvp_name _cvp_file _cvp_k _cvp_v _cvp_line 2>/dev/null; return 0; }
[[ -n "$_cvp_name" ]] || { unset -f _cvp_resolve_profile _cvp_unset_key 2>/dev/null; unset _cvp_dir _cvp_name 2>/dev/null; return 0; }
_cvp_file="$_cvp_dir/profiles/$_cvp_name.env"
if [[ ! -f "$_cvp_file" ]]; then unset -f _cvp_resolve_profile _cvp_unset_key 2>/dev/null; unset _cvp_dir _cvp_name _cvp_file 2>/dev/null; return 0; fi
if [[ -f "$_cvp_dir/profiles/.env-managed" ]]; then
  while IFS= read -r _cvp_managed_key || [[ -n "$_cvp_managed_key" ]]; do
    _cvp_unset_key "$_cvp_managed_key"
  done < "$_cvp_dir/profiles/.env-managed"
fi
for _cvp_managed_file in "$_cvp_dir"/profiles/*.env; do
  [[ -f "$_cvp_managed_file" ]] || continue
  while IFS= read -r _cvp_line || [[ -n "$_cvp_line" ]]; do
    _cvp_line="${_cvp_line#"${_cvp_line%%[![:space:]]*}"}"
    _cvp_line="${_cvp_line%"${_cvp_line##*[![:space:]]}"}"
    [[ -z "$_cvp_line" || "$_cvp_line" == \#* || "$_cvp_line" != *=* ]] && continue
    _cvp_managed_key="${_cvp_line%%=*}"
    _cvp_unset_key "$_cvp_managed_key"
  done < "$_cvp_managed_file"
done
while IFS= read -r _cvp_line || [[ -n "$_cvp_line" ]]; do
  _cvp_line="${_cvp_line#"${_cvp_line%%[![:space:]]*}"}"
  _cvp_line="${_cvp_line%"${_cvp_line##*[![:space:]]}"}"
...
unset -f _cvp_resolve_profile _cvp_unset_key 2>/dev/null
unset _cvp_dir _cvp_name _cvp_file _cvp_k _cvp_v _cvp_q _cvp_nl _cvp_line _cvp_managed_key _cvp_managed_file 2>/dev/null
RESOLVER
}
...
# Print cleanup and export commands for the resolved (or named) profile.
...
cvp_env() {
  ...
  local key
  while IFS= read -r key || [[ -n "$key" ]]; do
    _cvp_valid_env_key "$key" && printf "unset '%s'\n" "$key"
  done < "$CVP_ENV_MANAGED_VARS"
  local pair k v
  while IFS= read -r -d '' pair; do
    k="${pair%%=*}"; v="${pair#*=}"
...
${BOLD}COMMANDS${RESET}
  ...
  ${BOLD}env${RESET} [name]            Print cleanup/export commands for \`eval\` (real values)
